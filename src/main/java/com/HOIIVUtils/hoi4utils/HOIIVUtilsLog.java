...
public class HOIIVUtilsLog {
	...
	// return;
	// // throw new RuntimeException("HOIIVUtils directory does not exist/could not
	// be
	// // created");
	// }
	...
	// try (FileWriter fileWriter = new FileWriter(log, true); // true for append
	// mode
	...
	// } catch (IOException e) {
	// writeToLog("writeToLog has caused a error lol, probly cause the log file
	...
	// bufferedWriter.newLine();
	// } catch (IOException e) {
	// writeToLog("writeToLogError has caused a error lol, probly cause the log file
	// doesn't exist or something",
	// "ERROR");
	...
	// File nlogsDir = new File(SettingsManager.HOI4UTILS_PROPERTIES_PATH +
	// File.separator + "logs");
	// File hoi4utils_file = new File(SettingsManager.HOI4UTILS_PROPERTIES_PATH);
	// if (!hoi4utils_file.exists()) {
	// return null;
